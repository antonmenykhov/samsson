...
<div v-touch="swipe" @click.stop="closeLightBox">
    <transition mode="out-in" name="vib-container-transition" @afterEnter="enableImageTransition" @beforeLeave="disableImageTransition">
        <div v-if="media && media.length > 0" v-show="lightBoxShown" ref="container" class="vib-container">
            <div class="vib-content" @click.stop>
                <transition mode="out-in" :name="imageTransitionName">
                    ...
                    <div v-else-if="media[select].type == 'youtube'" class="video-background">
                        <iframe :src="'https://www.youtube.com/embed/' + media[select].id + '?showinfo=0'" width="560" height="315" frameborder="0" allowfullscreen />
                    </div>
                    <video v-else-if="currentMedia.type == 'video'" :key="currentMedia.sources[0].src" ref="video" controls :width="currentMedia.width" :height="currentMedia.height" :autoplay="currentMedia.autoplay">
                        <source v-for="source in currentMedia.sources" :key="source.src" :src="source.src" :type="source.type">
                    </video>
                </transition>
            </div> <!-- .vib-content -->

            <div v-if="showThumbs" class="vib-thumbnail-wrapper vib-hideable" :class="{ 'vib-hidden': controlsHidden }" @click.stop @mouseover="interfaceHovered = true" @mouseleave="interfaceHovered = false">
                <div v-for="(image, index) in imagesThumb" v-show="index >= thumbIndex.begin && index <= thumbIndex.end" :key="typeof image.thumb === 'string' ? `${image.thumb}${index}` : index" :style="{ backgroundImage: 'url(' + image.thumb + ')' }" :class="'vib-thumbnail' + (select === index ? '-active' : '')" @click.stop="showImage(index)">
            ...
            <button v-if="media.length > 1" type="button" class="vib-arrow vib-arrow-left vib-hideable" :class="{ 'vib-hidden': controlsHidden }" :title="previousText" @click.stop="previousImage()" @mouseover="interfaceHovered = true" @mouseleave="interfaceHovered = false">
                <slot name="previous">
                    <LeftArrowIcon />
                </slot>
            </button>

            <button v-if="media.length > 1" type="button" class="vib-arrow vib-arrow-right vib-hideable" :class="{ 'vib-hidden': controlsHidden }" :title="nextText" @click.stop="nextImage()" @mouseover="interfaceHovered = true" @mouseleave="interfaceHovered = false">
                <slot name="next">
                    <RightArrowIcon />
                </slot>
            </button>
